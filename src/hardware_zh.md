...
也许, 我们知道一件事情发生的时候发生了什么, 下一次这样的事情发生, 就是我们故意的了.
...
分配, 销毁, 释放, 放一起叫内存管理, 这可是个大讲究.

有些语言,比如C, Rust, 需要手动管理, 意味着你需要把释放体现在代码上. 人们讨论语言性能到最后, 就是如何安全地手动管理内存. 

民用语言大都内置了垃圾回收(GC)程序, GC会追踪无法被调用的对象, 并将它们占有的内存释放掉. 一个很简单的例子:
...
两秒之后, `[1,2,3,4,5]` 的名字被夺走, 这串数据成了孤岛, 编程者无法再调用它, 它已经被销毁, 存在的意义只剩下占用内存, GC会自动找到它, 将这一块内存释放. 

GC虽好, 但它是你的程序里一个不受你控制的程序, 要是它在你不想卡的时候卡那么一下呢?
...
你可以把内存切的更细, 将不要紧的部分踢给别的CPU去做. 

过一会儿拿回来, 或者不拿回来, 就需要编程者体现在代码里了, 这个叫「异步」

不同的语言存在不同的多核调动方式, 其中async await相对简洁. 一个很简单的例子:
...
出现在await 的函数都被标记为 async, 反过来说同样成立. 
```swift
func 包饺子() async -> 饺子 {
...
- 在func要来的这块内存上划出一块来, 放饺子皮, 这块内存要等其他的cpu擀出来.

- 等饺子皮儿到了, 我放上馅, 一捏, 做好一个饺子, 这块内存就剩这个饺子了. 
